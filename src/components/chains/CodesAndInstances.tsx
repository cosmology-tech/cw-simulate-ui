...
const CodesAndInstances = () => {
  ...
  const handleInstantiate = () => {
    if (payload.length === 0) {
      showNotification(setSnackbarNotification, "Payload cannot be empty", "error");
      return;
    }
    ...
    if (!binary) {
      showNotification(setSnackbarNotification, "Failed to extract WASM bytecode", "error");
      return;
    }
      ...
      if (chain === param.id) {
        createContractInstance(window.CWEnv.chains[chain], wasmBytes as Buffer).then((contract) => {
          try {
            ...
            showNotification(setSnackbarNotification, "Unable to instantiate with " + e, "error");
          }
        });
      }
    }
